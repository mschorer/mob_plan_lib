package de.ms_ite.maptech.cache {
	...
	public class ContentCachePersistent extends EventDispatcher implements IContentLoader {
		
		public static var dbFile:String = "contentCache.sq3";
		protected static var _tableName:String = "cache";
		
		protected var statementMap:Object = new Object();		
		...
		public function ContentCachePersistent() {
			super();
		...
		protected function initLocalDB():Boolean {
			...
			if ( dbConn == null) {
				...
				//			dbConn.addEventListener( SQLErrorEvent.ERROR, openDBFailure);
				//			dbConn.addEventListener(SQLEvent.BEGIN, beginDBHandler);
				
				var dbf:File = AppSettings.persistantStorage.resolvePath( dbFile);
				dbConn.open( dbf);
				
				freshlyOpened = true;
				trace( "db inited. ["+dbFile+"]");
			}
		...
		protected function getSql( tag:String):String {
			...
			switch( tag) {
				...
				case 'insert':
					sql = "INSERT INTO "+_tableName+" ( tag, filename) VALUES ( :TAG, :FILENAME)";
					break;
				case 'update':
					sql = "UPDATE "+_tableName+" SET last_use=current_timestamp WHERE id=:ID";
					break;
				...
				case 'list':
					...
					sql = "SELECT * FROM "+_tableName+" where 1 ORDER BY last_use DESC";
					break;
				case 'clear':
		...
		protected function addSqlParameters( stmt:SQLStatement, tag:String, parms:Object=null):void {
			...
			switch( tag) {
				...
				case 'update':
					stmt.parameters[':TAG'] = parms.tag;
					stmt.parameters[':FILENAME'] = parms.filename;
					break;
				
				case 'update':				
				case 'delete':
				case 'select':
					stmt.parameters[':ID'] = parms.id;
					break;
		...
		public function initCache():Boolean {
			var sqe:SQLResult = executeOperation( 'list' /*, parms */);
			
			for( var i:int = 0; i < sqe.data.length; i++) {
				var o:Object = sqe.data[ i];
				
				_fileCache[ o.tag] = o.filename;
			}
		...
		public function saveDB():SQLResult {
			debug( "DBsave["+existsInDB+"]: "+toString());
		...
		protected function insertDB():SQLResult {
			...
			var id:int = sqe.lastInsertRowID;
			debug( "  DBinsert: "+id);
