package de.ms_ite.maptech.cache {
	...
	public class TileInfoCaching extends TileInfo {
		...
		public function TileInfoCaching( basePath:File=null, img:Image=null, mi:MapInfo=null, lay:int=-1, x:int=-1, y:int=-1) {
...
//			if ( _local_url != null) return true;
		...
		public function save( buffer:ByteArray):String {
			...
			if ( buffer.length < 5) return null;
				...
				var first:String = buffer.readUTFBytes( 5);
				
				if ( first == '<?xml' || first == '<html') {
//					debug( "error #["+buffer.length+"] ["+first+((buffer.length > 0) ? buffer.readUTFBytes( Math.min( buffer.bytesAvailable, 512)) : '')+"]");
//					debug( "error.");
					return null;
				}
			}
		...
		protected function getSql( tag:String):String {
			...
			switch( tag) {
				...
				case 'list':
					...
					break;
				case 'clear':
					sql = "DELETE FROM "+_tableName+" WHERE map=:MAP";
					break;
...
//			debug( "  DBdelete: "+sqe);
		...
		public function clearLayerDB():SQLResult {
			var sqe:SQLResult = executeOperation( 'clear');			
			debug( "  DBclear["+_mapInfo.name+"]: "+sqe);

			var tileStore:File = _basePath.resolvePath( 'cache');
			var layerStore:File = tileStore.resolvePath( _mapInfo.name);
			if ( layerStore.exists && layerStore.isDirectory) layerStore.deleteDirectory( true);
		...
		public function list():ArrayCollection {
			...
			if ( sqe.data != null) {
				for( var i:int = 0; i < sqe.data.length; i++) {
					var ti:TileInfoCaching = new TileInfoCaching();
